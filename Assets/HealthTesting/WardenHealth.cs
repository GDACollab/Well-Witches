...
public class WardenHealth : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Fetch health from WardenStatManagement
        if (WardenStatsManagement.Instance != null)
        {
            maxHealth = WardenStatsManagement.Instance.GetHealth();
        }
        else
        {
            Debug.LogError("❌ WardenStatManagement Instance is missing!");
    ...
    void CheckObstacleDamage(GameObject obstacle)
    {
        ...
        {
            Debug.Log($"Warden hit {obstacle.name} and took {damage} damage.");
            ReduceHealth(damage);
        }
        else
        {
    ...
    void UpdateHealthBar()
{
    ...
    Debug.Log($"📏 Warden Health bar updated: {currentHealth}/{maxHealth} | Width: {fullWidth} -> {newWidth}");
}
